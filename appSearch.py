...
#         uploaded_embedding = siamese_network.get_layer('Encoder').predict(img_array)
...
#             db_img_array = np.expand_dims(db_img_array, axis=0)  # Convert shape to (1, 128, 128, 3)
#             db_img_array = db_img_array / 255.0  # Normalize the image

#             # Get embeddings for the database image
#             db_embedding = siamese_network.get_layer('Encoder').predict(db_img_array)

#             # Compute the Euclidean distance between the embeddings
#             distance = np.linalg.norm(uploaded_embedding - db_embedding)
#             logging.debug(f"Distance between uploaded image and {img_path}: {distance}")
...
#                 "found": "yes",
#                 "image": best_match,
#                 "sim": float(best_distance)  # Convert float32 to float
#             }
#         else:
#             response = {"found": "no"}
...
app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = 'C:/laragon/www/fmp/uploads'
...
try:
    siamese_network = get_siamese_network()
    siamese_network.load_weights('C:/laragon/www/fmp/model/siamese_92.weights.h5', by_name=True, skip_mismatch=False)
except Exception as e:
...
db_config = {
    ...
    "password": "",
    "database": "children_db",
    "port": 3306  # Replace with your new port number if changed
}
...
def save_to_db(input_data, output_data):
    try:
        ...
        cursor = conn.cursor()
        sql = "INSERT INTO predictions (input_data, output_data) VALUES (%s, %s)"
        cursor.execute(sql, (input_data, output_data))
        conn.commit()
        cursor.close()
        conn.close()
        return True
...
def predict():
    try:
        ...
        # Get embeddings for the uploaded image
        encoder = siamese_network.get_layer('Encoder')
        uploaded_embedding = encoder.predict(img_array)
        ...
        for img_path in all_images:
            ...
            # Get embeddings for the database image
            db_embedding = encoder.predict(db_img_array)
